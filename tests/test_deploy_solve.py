...
from blab.deploy_solve import DEPLOY_SOLVE_SCHEMA, prepare_deploy_solve_request
...
def test_prepare_deploy_solve_request_stages_lod_trace_and_grid(tmp_path: Path) -> None:
    ...
    assert request["provenance"]["node_count"] == 1290
    assert request["provenance"]["face_count"] == 2576
    assert Path(request["mesh_file"]).is_file()
    assert len(request["boundary_neumann"]["real"]) == 2576
    assert len(request["reference_boundary_pressure"]["real"]) == 1290
    ...
    assert request["source_transform"] == {
    ...
    actual = np.asarray(request["boundary_neumann"]["real"], dtype=np.float32) + 1j * np.asarray(
        request["boundary_neumann"]["imag"], dtype=np.float32
    )
