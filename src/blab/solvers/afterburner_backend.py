"""Afterburner solver backend adapter."""
...
DEFAULT_AFTERBURNER_SOLVER_SCRIPT = Path(__file__).with_name("julia_local") / "solver.jl"
DEFAULT_AFTERBURNER_PROJECT = DEFAULT_AFTERBURNER_SOLVER_SCRIPT.parent
_WORKERS_LOCK = threading.Lock()
_WORKERS: dict[tuple[str, str, str, str], "AfterburnerWorkerProcess"] = {}


class AfterburnerSession:
    def __init__(
        self,
        request_payload: SolveRequest,
        *,
        julia_executable: str = "julia",
        solver_script: str | Path = DEFAULT_AFTERBURNER_SOLVER_SCRIPT,
        julia_threads: str | int = "auto",
        julia_project: str | Path | None = DEFAULT_AFTERBURNER_PROJECT,
        ...
        self._stop = False
        self._temp_dir = tempfile.TemporaryDirectory(prefix="blab-afterburner-")
        self._process: subprocess.Popen[str] | None = None
        self._worker: AfterburnerWorkerProcess | None = None
    ...
    def metadata(self) -> SolveMetadata:
        if self._metadata is None:
            raise RuntimeError("Afterburner solver session has not initialized.")
        ...
        try:
            for event in self._events:
                ...
                if event_type == "result":
                    ...
                    raise RuntimeError(str(event.get("error", "Afterburner solver failed.")))
        finally:
            self._close()
    ...
    def _start_and_initialize(self) -> None:
        if not self.solver_script.exists():
            raise RuntimeError(f"Afterburner solver script does not exist: {self.solver_script}")
        ...
        for event in self._events:
            ...
            if event_type == "status":
                ...
                raise RuntimeError(f"Afterburner solver ended before initialization: {event_type}")

        raise RuntimeError(self._process_error("Afterburner solver ended before initialization."))
    ...
    def _iter_events(self) -> Iterator[dict]:
        ...
        if exit_code != 0 and not self._stop:
            raise RuntimeError(self._process_error(f"Afterburner solver exited with code {exit_code}."))
...
class AfterburnerWorkerProcess:
    def __init__(
        self,
        *,
    ...
    def submit(self, request_path: Path) -> Iterator[dict]:
        ...
        try:
            ...
            if process is None or process.stdin is None:
                raise RuntimeError("Warm Afterburner solver did not provide stdin.")
    ...
    def _ensure_started(self) -> None:
        ...
        for event in self._read_events():
            ...
            if event_type == "failed":
                raise RuntimeError(str(event.get("error", "Afterburner solver failed during startup.")))

        raise RuntimeError(self._process_error("Warm Afterburner solver ended before startup completed."))
        ...
        try:
            for event in self._read_events():
                ...
                if str(event.get("type", "")) in {"completed", "cancelled", "failed"}:
                    return
            raise RuntimeError(self._process_error("Warm Afterburner solver ended before job completion."))
        finally:
            if self._lock.locked():
                self._lock.release()
    ...
    def _read_events(self) -> Iterator[dict]:
        ...
        self._process = None
        if exit_code != 0:
            raise RuntimeError(self._process_error(f"Warm Afterburner solver exited with code {exit_code}."))
...
class AfterburnerBackend:
    ...
    def create_session(self, request: SolveRequest) -> AfterburnerSession:
        return AfterburnerSession(
            request,
            julia_executable=self.julia_executable,
            solver_script=self.solver_script,
            julia_threads=self.julia_threads,
...
def _get_julia_worker(
    *,
    ...
    with _WORKERS_LOCK:
        ...
        if worker is None:
            worker = AfterburnerWorkerProcess(
...
def shutdown_afterburner_workers() -> None:
    with _WORKERS_LOCK:
...
DEFAULT_JULIA_SOLVER_SCRIPT = DEFAULT_AFTERBURNER_SOLVER_SCRIPT
DEFAULT_JULIA_PROJECT = DEFAULT_AFTERBURNER_PROJECT
JuliaLocalSession = AfterburnerSession
JuliaWorkerProcess = AfterburnerWorkerProcess
JuliaLocalBackend = AfterburnerBackend
shutdown_julia_workers = shutdown_afterburner_workers
