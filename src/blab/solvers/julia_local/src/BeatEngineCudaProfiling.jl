...
function _cuda_regular_quadrature_slp_kernel!(
    slp_re,
    slp_im,
    face_vertices,
    areas,
    ...
    pair_index = blockIdx().x
    pair_index > total_pairs && return nothing

    test_local = ((pair_index - 1) % length(test_indices)) + 1
    trial_local = div(pair_index - 1, length(test_indices)) + 1
    test_index = test_indices[test_local]
    trial_index = trial_indices[trial_local]
...
function _cuda_regular_quadrature_hyp_kernel!(
    hyp_re,
    hyp_im,
    face_vertices,
    normals,
    ...
    tid = threadIdx().x
    stride = blockDim().x
    scratch[tid + 0 * stride] = hyp11_re
    scratch[tid + 1 * stride] = hyp12_re
    scratch[tid + 2 * stride] = hyp13_re
    scratch[tid + 3 * stride] = hyp21_re
    scratch[tid + 4 * stride] = hyp22_re
    scratch[tid + 5 * stride] = hyp23_re
    scratch[tid + 6 * stride] = hyp31_re
    scratch[tid + 7 * stride] = hyp32_re
    scratch[tid + 8 * stride] = hyp33_re
    scratch[tid + 9 * stride] = hyp11_im
    scratch[tid + 10 * stride] = hyp12_im
    scratch[tid + 11 * stride] = hyp13_im
    scratch[tid + 12 * stride] = hyp21_im
    scratch[tid + 13 * stride] = hyp22_im
    scratch[tid + 14 * stride] = hyp23_im
    scratch[tid + 15 * stride] = hyp31_im
    scratch[tid + 16 * stride] = hyp32_im
    scratch[tid + 17 * stride] = hyp33_im
    sync_threads()
    ...
    if threadIdx().x == 1
        _cuda_atomic_add!(hyp_re, t1 + (r1 - 1) * p1_dof_count, scratch[1 + 0 * stride])
        _cuda_atomic_add!(hyp_re, t1 + (r2 - 1) * p1_dof_count, scratch[1 + 1 * stride])
        _cuda_atomic_add!(hyp_re, t1 + (r3 - 1) * p1_dof_count, scratch[1 + 2 * stride])
        _cuda_atomic_add!(hyp_re, t2 + (r1 - 1) * p1_dof_count, scratch[1 + 3 * stride])
        _cuda_atomic_add!(hyp_re, t2 + (r2 - 1) * p1_dof_count, scratch[1 + 4 * stride])
        _cuda_atomic_add!(hyp_re, t2 + (r3 - 1) * p1_dof_count, scratch[1 + 5 * stride])
        _cuda_atomic_add!(hyp_re, t3 + (r1 - 1) * p1_dof_count, scratch[1 + 6 * stride])
        _cuda_atomic_add!(hyp_re, t3 + (r2 - 1) * p1_dof_count, scratch[1 + 7 * stride])
        _cuda_atomic_add!(hyp_re, t3 + (r3 - 1) * p1_dof_count, scratch[1 + 8 * stride])
        _cuda_atomic_add!(hyp_im, t1 + (r1 - 1) * p1_dof_count, scratch[1 + 9 * stride])
        _cuda_atomic_add!(hyp_im, t1 + (r2 - 1) * p1_dof_count, scratch[1 + 10 * stride])
        _cuda_atomic_add!(hyp_im, t1 + (r3 - 1) * p1_dof_count, scratch[1 + 11 * stride])
        _cuda_atomic_add!(hyp_im, t2 + (r1 - 1) * p1_dof_count, scratch[1 + 12 * stride])
        _cuda_atomic_add!(hyp_im, t2 + (r2 - 1) * p1_dof_count, scratch[1 + 13 * stride])
        _cuda_atomic_add!(hyp_im, t2 + (r3 - 1) * p1_dof_count, scratch[1 + 14 * stride])
        _cuda_atomic_add!(hyp_im, t3 + (r1 - 1) * p1_dof_count, scratch[1 + 15 * stride])
        _cuda_atomic_add!(hyp_im, t3 + (r2 - 1) * p1_dof_count, scratch[1 + 16 * stride])
        _cuda_atomic_add!(hyp_im, t3 + (r3 - 1) * p1_dof_count, scratch[1 + 17 * stride])
    end
