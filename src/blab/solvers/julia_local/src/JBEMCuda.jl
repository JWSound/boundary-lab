...
function assemble_regular_galerkin_operators_cuda_regular(
    ...
    _cuda_timed_stage!(timing, "regular_operator_kernel") do
        if parallel_quadrature
            threads = max(32, min(128, nextpow(2, rule_count * rule_count)))
            CUDA.@cuda threads=threads blocks=total_pairs shmem=threads * 48 * sizeof(T) _cuda_regular_quadrature_kernel!(
                slp_re,
                slp_im,
                dlp_re,
                dlp_im,
