...
function _rocm_regular_slp_adjoint_pairs_kernel!(
    single_layer,
    ...
    while test_q <= rule_count
        ...
        test_basis = SVector(one(k) - test_xi - test_eta, test_xi, test_eta)
    ...
    column = element_dp0_dofs[trial_index]
    local_row = 1
    while local_row <= 3
...
function _rocm_regular_dlp_hyp_pairs_kernel!(
    double_layer,
    hypersingular,
    face_vertices,
    normals,
...
function _rocm_colored_pair_block_count(cache::RocmRegularAssemblyCache)
    groupsize = _rocm_kernel_groupsize()
    blocks = 0
    color_count = length(cache.color_offsets) - 1
    for test_color in 1:color_count, trial_color in 1:color_count
        test_count = cache.color_offsets[test_color + 1] - cache.color_offsets[test_color]
        trial_count = cache.color_offsets[trial_color + 1] - cache.color_offsets[trial_color]
        blocks += cld(test_count * trial_count, groupsize)
    end
    return blocks
end
