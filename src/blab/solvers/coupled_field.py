...
def evaluate_bem_field(
    ...
    *,
    ...
    if normalized_backend == "beat_remote" or not supports_physical_system_solves(normalized_backend):
        raise ValueError("Retained BEM fields require a local BEAT Engine backend.")
    bem_backend = _bem_backend_for_id(normalized_backend)
    julia_project = default_beat_engine_project(bem_backend)
